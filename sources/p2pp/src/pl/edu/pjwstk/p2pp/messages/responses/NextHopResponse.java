...
public class NextHopResponse extends Response {
	...
	public String toString() {
		StringBuilder builder = new StringBuilder("NextHopResponse=[message=[" + super.toString() + "header=["
				+ getHeader() + "], senderPeerInfo=[");
	 ...
	 * @param nextHop
	 */
	public NextHopResponse(boolean[] protocolVersion, boolean isSentByPeer, boolean isRecursive,
			byte requestOrResponseType, byte ttl, byte[] transactionID, byte[] sourceID, byte[] responseID,
			boolean isOverReliable, boolean isEncrypted, PeerInfo senderPeerInfo, PeerInfo nextHop) {
		super(protocolVersion, P2PPMessage.RESPONSE_ACK_MESSAGE_TYPE, false, isSentByPeer, isRecursive,
				RESPONSE_CODE_NEXT_HOP_BITS_ARRAY, requestOrResponseType, ttl, transactionID, sourceID, responseID,
	...
	public void addObject(GeneralObject object) throws UnsupportedGeneralObjectException {
		if (object instanceof PeerInfo) {
			...
			throw new UnsupportedGeneralObjectException("NextHopResponse can't contain " + object.getClass().getName()
					+ " as subobject.");
		}
	}
	...
	protected byte[] asBytes(int bitsCount) {
		...
		// currentIndex += nextHopPeerInfo.getBitsCount();
