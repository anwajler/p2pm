 ...
 * @author Piotr Bucior
 */
public class ConsistencyManager extends AbstractConsistencyManager {
     ...
     */
    protected JCsyncAlgorithInterface core;
    private final ConcurrentHashMap<String, ArrayList<JCsyncAbstractOperation>> operationBuffer;

    /**
     * Creates new instance of <tt>DefaultConsistencyManager</tt>
     */
    public ConsistencyManager() {
        operationBuffer = new ConcurrentHashMap<String, ArrayList<JCsyncAbstractOperation>>();
    }
    ...
    public void indicationReceived(JCsyncAbstractOperation op) {
        ...
        if (this.blockingRequests.containsKey(op)) {
            log.trace(this.core.getNodeInfo().getName() + ": (indicationReceived) - local suspended thread for: " + op.toString());
...
//                    }
            ...
            //synchronized (operationBuffer) {
                if (this.operationBuffer.containsKey(op.getObjectID())) {
                    log.trace(this.core.getNodeInfo().getName() + ": no registered notifyConsumer yet, passing operation to the buffer: " + op.toString());                            this.operationBuffer.get(op.getObjectID()).add(op);
                    b = true;
                } 
           // }
            if(!b){
                    this.requestManagers.get(op.getObjectID()).nc.deliveredIndications.add(op);
                    log.trace(this.core.getNodeInfo().getName() + ": (indicationReceived) - passed to notify consumer: " + op.toString());
//                    synchronized (this.requestManagers.get(op.getObjectID()).nc.locker) {
//                        this.requestManagers.get(op.getObjectID()).nc.locker.notifyAll();
//                    }
                }
            log.trace(this.core.getNodeInfo().getName() + ": (indicationReceived) - ends managing operation: " + op.toString());
        }
    }
            ...
            while (true) {
                ...
                if (dr.operation.getOperationType() == OP_REQ_LOCK_APPLY) {
                    ...
                    try {
                        ...
                        ((OperationDetails)mc.getArgValues()[1]).setRequestInvokingTime(System.currentTimeMillis());  
                    ...
                    if ((op.getOperationType() + 128 & OP_INDICATION_GENERIC) == OP_INDICATION_GENERIC) {
                        ...
                        // -------------------------------------
                           ...
                           // }
                                ...
                                log.error("An error occurred for operation: "+op.toString(),e);
                                log.trace("BlockingRequest map values: "+blockingRequests);
                            }
                        } else {
                            blockingRequests.get(op).retVal = op.getDetails();
                            //release semaphore only when the respCode already is set up;                    
