...
public class Acknowledgment extends P2PPMessage {
	 ...
	 * @param isEncrypted
	 */
	public Acknowledgment(boolean[] protocolVersion, boolean[] messageType, boolean isSentByPeer, boolean isRecursive,
			boolean[] reservedOrResponseCode, byte requestOrResponseType, byte ttl, byte[] transactionID,
			byte[] sourceID, byte[] responseID, boolean isOverReliable, boolean isEncrypted) {
		super(protocolVersion, messageType, true, isSentByPeer, isRecursive, reservedOrResponseCode,
	...
	public PeerInfo getPeerInfo() {
		// Acknowledgment doesn't contain any subobjects, particularly PeerInfo.
		return null;
	}
