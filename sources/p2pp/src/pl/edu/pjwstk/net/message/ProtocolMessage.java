...
public abstract class ProtocolMessage extends ProtocolObject implements ProtocolMessageInt{
	...
	public boolean generateTransactionID(int lengthOfTransactionID) {
		...
		setTransactionID(new ExtendedBitSet(
				Integer.toBinaryString((int)(Math.random()*Integer.MAX_VALUE)),
                                lengthOfTransactionID,false,false));
	...
	public ExtendedBitSet getHeader(){
		...
		//logger.info(header.getFixedLength());
		header.add(protocolBits);
		//logger.info(header.getFixedLength());
		header.add(protocolMessageType);
		//logger.info(protocolMessageType.getFixedLength() + " ");
		header.add(protocolMessageLength);
		//logger.info(protocolMessageLength.getFixedLength());
		header.add(protocolMagicCookie);
		//logger.info(protocolMagicCookie.getFixedLength());
		header.add(getTransactionID());
		//logger.info(transactionID.getFixedLength());
		//logger.info(header.getFixedLength());
