...
public enum TURNMessageClass{
	...
	TURNMessageClass(String bitString){
		try{
	...
	public static TURNMessageClass valueOf(ExtendedBitSet messClassBitSet) throws Exception {
		if (messClassBitSet.getFixedLength() != 2) {
			throw new Exception ("The size of BitSet must equal 2. (" + messClassBitSet.length() + ")" );
		}		
		for (TURNMessageClass messClass : TURNMessageClass.values()) {
