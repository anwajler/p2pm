 ...
 * @author Doug Lea
 */
public class TransportThreadPoolExecutor extends AbstractExecutorService {
    ...
    static class WorkerComparator implements Comparator {
        public int compare(Object worker1, Object worker2) {
            Worker w1 = (Worker) worker1;
            Worker w2 = (Worker) worker2;
            int result = 1;
            if (w1.getAddressCount() == w2.getAddressCount()) result = 0;
            if (w1.getAddressCount() < w2.getAddressCount()) result = -1;
            return result;
        }
    }
     ...
     * holding mainLock.
     */
    private final TreeSet<Worker> workers = new TreeSet<Worker>(new WorkerComparator());
     ...
     */
    private volatile TransportRejectedExecutionHandler handler;
     ...
     */
    private static final TransportRejectedExecutionHandler defaultHandler =
        new AbortPolicy();
     ...
     */
    public TransportThreadPoolExecutor(int corePoolSize,
                              int maximumPoolSize,
                              long keepAliveTime,
                              TimeUnit unit,
    ...
    public TransportThreadPoolExecutor(int corePoolSize,
                              ...
                              TimeUnit unit,
                              BlockingDeque<Runnable> workQueue,
                              TransportRejectedExecutionHandler handler) {
        this(corePoolSize, maximumPoolSize, keepAliveTime, unit, workQueue,
    ...
    public void execute(Runnable command) {
        ...
        if (poolSize >= corePoolSize || !addIfUnderCorePoolSize(command)) {
            if (runState == RUNNING && workQueue.offerFirst(command)) {
    ...
    private Thread addThread(Runnable firstTask) {
        Worker w = new Worker(firstTask, this.workerId.incrementAndGet());
    ...
    private final class Worker implements Runnable {
        ...
        final private List<String> addresses = Collections.synchronizedList(new ArrayList<String>());
        final private AtomicInteger addressesCount = new AtomicInteger(0);

        Worker(Runnable firstTask, int id) {
        ...
        public void run() {
            try {
                ...
                while (task != null || (task = getTask(this)) != null) {
                    ...
                    for (String address : this.addresses) sb.append(address + ";");
                    System.out.println(sb.toString());*/
                    if (task instanceof FutureSendTask) {
                        FutureSendTask fst = (FutureSendTask) task;
                        SendTask ts = (SendTask) fst.getRunnable();
                        String address = ts.getMessageReceiverAddress();
        ...
        public void addAddress(String address) {
            ...
            this.addressesCount.incrementAndGet();
        }
        ...
        public int getAddressCount() {
            return this.addressesCount.get();
        }
    }

    /* Utilities for worker thread control */

    private static Runnable emptyTask = new Runnable() {
        public void run () {}
    };
        ...
        for (;;) {
            try {
                ...
                if (state == SHUTDOWN)  // Help drain queue
                    r = workQueue.pollLast();
                else if (poolSize > corePoolSize || allowCoreThreadTimeOut)
                    r = workQueue.pollLast(keepAliveTime, TimeUnit.NANOSECONDS);
                else {
                    //boolean notHasAddress = true;
                    //while (notHasAddress) {
                        r = workQueue.takeLast();
                        //System.out.println(r);
                        //System.out.println("worker"+worker.getId());
                        if (r != null) {

                            FutureSendTask fst = (FutureSendTask) r;
                            SendTask st = (SendTask) fst.getRunnable();
                            String stAddress = st.getMessageReceiverAddress();
                            //System.out.println(stAddress);

                            boolean noWorkerHasAddress = true;
                            for (Worker w : this.workers) {
                                if (w == worker) continue;
                                if (w.hasAddress(stAddress)) {
                                    noWorkerHasAddress = false;
                                    break;
                                }
                            }
                            //System.out.println(noWorkerHasAddress);

                            if (noWorkerHasAddress) {
                                //notHasAddress = false;
                                /*if (worker.hasAddress(stAddress)) {
                                    System.out.println(notHasAddress+"-");
                                } else {
                                    System.out.println(notHasAddress+"+");
                                } */
                                return r; // This worker takes the task
                            } else {
                                this.workQueue.putLast(r); // Some other worker got it - returning it to the queue
                                return TransportThreadPoolExecutor.emptyTask;
                            }
     ...
     */
    public void setRejectedExecutionHandler(TransportRejectedExecutionHandler handler) {
        if (handler == null)
            throw new NullPointerException();
        this.handler = handler;
    }
     ...
     */
    public TransportRejectedExecutionHandler getRejectedExecutionHandler() {
        return handler;
    }
     ...
     */
    public static class AbortPolicy implements TransportRejectedExecutionHandler {
        /**
         ...
         */
        public void rejectedExecution(Runnable r, TransportThreadPoolExecutor e) {
     ...
     * rejected task.
     */
    public static class DiscardPolicy implements TransportRejectedExecutionHandler {
        /**
    ...
    protected <T> RunnableFuture<T> newTaskFor(Runnable runnable, T value) {
        return new FutureSendTask<T>(runnable, value);
    }

    static class FutureSendTask<V> extends FutureTask<V> {
        private Runnable runnable;
        public FutureSendTask(Callable<V> callable) {
            super(callable);
        }
        public FutureSendTask(Runnable runnable, V result) {
    ...
    /*static class TransportThreadFactory implements ThreadFactory {
        ...
        public Thread newThread(Runnable r) {
            Thread t = new TransportThread(group, r,
    ...
    static class TransportThread extends Thread {
	    private List<String> addresses = Collections.synchronizedList(new ArrayList<String>());
        public TransportThread(ThreadGroup group, Runnable target, String name, long stackSize) {
...
interface TransportRejectedExecutionHandler {
     ...
     */
    void rejectedExecution(Runnable r, TransportThreadPoolExecutor executor);
}