...
public final class ByteUtils {
	...
	private static final String INDEX_OUT_OF_BOUNDS_MESSAGE = "You're trying "
			+ "to write some bits before or after the array.";
...
/*
		...
		for (int i = 0; i < array.length; i++) {
			arrayString += Integer.toBinaryString(array[i] & 0xFF) + " ";
		}
*/
                
		// System.out.println("Do tablicy wpisano bajt="
		// + Integer.toBinaryString(b & 0xFF) + " na indeksie=" + bitIndex
		// + "; tablica po=" + arrayString);
	...
	public static byte copyBitsToByte(byte first, byte second, int firstIndex, int secondIndex) {
		...
		// If first bits of second aren't erased by copying on them.
		if (secondIndex > 0) {
		...
		// Loop copying bits as long as both indexes are below 8.
	...
	public static void addByteArrayToArrayAtByteIndex(byte[] toBeCopied, byte[] toBeFilled, int byteIndex) {
		...
		if ((byteIndex + toBeCopiedLength) > toBeFilled.length) {
			throw new ArrayIndexOutOfBoundsException(
					"Array that has to be copied is to long to fit into array that has to be filled. toBeCopied.length="
							+ toBeCopiedLength + ", toBeFilled.length=" + toBeFilled.length + ", byteIndex="
							+ byteIndex);
		}
	...
	public static byte[] stringIPAddressToBytes(String address) {
		...
		// checks type of address... if "." used->IPv4, if ":"->IPv6
	...
	public static String bytesToStringIP(byte[] byteArray) {
		...
		return (byteArray[0] & 0xFF) + "." + (byteArray[1] & 0xFF) + "." + (byteArray[2] & 0xFF) + "."
				+ (byteArray[3] & 0xFF);
	}
